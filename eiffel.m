image1 = imread("images/eiffel.jpg");
image2 = imread("images/eiffel_past_cut.jpg");
...
image1 = rescale(imread("images/eiffel.jpg"));
image2 = rescale(imread("images/eiffel_past_cut.jpg"));
...
big_image(300:299+size(homography_im,1),300:299+size(homography_im,2),:) =...
...
big_image(300+translate_row:299+translate_row+size(image2,1),...
                300+translate_col:299+translate_col+size(image2,2),:) = image2;

imshow(big_image)

%%
inliers = zeros(size(col1,1),1);
...
%     half_size = 7;
    ...
    for i = (1:size(row))
    %         collect_area = L(row(i) - half_size:row(i) + half_size,col(i) - half_size:col(i) + half_size);
...
function [feature_descriptor] = get_descriptor(row, col, key_orientations, key_num, m, theta, dog_image)
    ...
    %                 sum(sub_window_m(sub_hist_theta == k),'all')
                end
            end
        end
%         feature_descriptor(l,:) = normalize(feature_finger_print);
...
%%
function [filt_features1,filt_features2, half_im1] = gauss_image(image1)
    height = size(image1,1);
    width = size(image1,2);

    scaled_images = zeros(5,height,width);
    for i = (1:5)
        G = fspecial('gaussian',round(3*sqrt(2)^i),sqrt(2)^i);
        scaled_images(i,:,:) = imfilter(image1,G);
    end

    dog = zeros(4,height, width);
    for i = (1:4)
        dog_image = squeeze(scaled_images(1,:,:) - scaled_images(2,:,:));
        dog(i,11:height-10,11:width-10) = dog_image(11:height-10,11:width-10);
    end
    ...
    half_im1 = imresize(squeeze(scaled_images(5,:,:)),0.5);
end
%%
...
% 
%     % get strong features
%     threshold = corner_threshold*mean(R,'all');
%     strong_features = R .* R>threshold;
%     strong_features = strong_features >0;

    disp(sum(sum(strong_features>0)));
end

%%
function nms_features = nms(image, filter_size)
    % make padding for nms
    col_padding = zeros(size(image,1),filter_size);
    padding_features = [col_padding, image, col_padding];
    row_padding = zeros(filter_size, size(padding_features, 2));
    padding_features = [row_padding; padding_features; row_padding];

    % do nms
    nms_features = zeros(size(image));
    circle = fspecial('disk',filter_size);
    circle_binary = circle>0;
    for row = 1:size(image,1)
        for col = 1:size(image,2)
            nms_features(row, col) = each_nms(padding_features,row+filter_size,col+filter_size,filter_size,circle_binary);
        end
    end
end
    
%% 
function ret = each_nms(image, row, col, radius, circle)
